...
#'
#' scCODE makes data-specific evaluation of different combination of gene filtering methods and DE methods scRNA-seq data,
#' and provides with data-specific optimal DE result.
...
#' @param top_ranked number of top-ranked strategies for consensus optimization, 5 <= top_ranked <= 10.
#' @return DE results, re-ranked DE results detected by the top-ranked solutions.
#' @return Z-score-metrics, Z-scores of the CDO and AUCC.
#' @author Jiawei Zou
#' @export
#' @examples
#'
#' set.seed(123)
#' test=scCODE(data_sccode,group_sccode,light=TRUE,top_ranked=5)
#' head(test$DE_results)
#'
scCODE<-function(data,group,light=TRUE,top_ranked=5){
  data_ori<-data
  group_ori<-group
  top_ranked<-stats::median(c(top_ranked,5,10))### top 5 - 20 consensus
  ...
  if(light==TRUE){
    DEmethods<-rev(c('t_test','BPSC','wilcox_test','DESeq2','limma','edgeR'))
    filtermethods<-c(2:3)
  }else{
  ...
  if(light==F){
    ...
    ###Z-score
    aucc2=scCODE_aucc(list_all[(1):(1*length(DEmethods))])
    aucc3=scCODE_aucc(list_all[(1*length(DEmethods)+1):(2*length(DEmethods))])

    o2=(colSums(aucc2)-1)/(length(DEmethods)-1)
    o3=(colSums(aucc3)-1)/(length(DEmethods)-1)
    AUCC_results=z_score(c(o2,o3))
  }
  ...
  idx_optimal=order(res_t,decreasing = T)[1:top_ranked]####top suitable strategies consensus
  consensus_res<-NULL
  info_all<-NULL
  ...
  #####save results/ return
  Results_all<-c(list(data.frame(consensus_res)),list(res_all))

  names(Results_all)<-c('DE_results','Z-score-metrics')

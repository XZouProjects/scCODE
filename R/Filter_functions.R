...
#' set.seed(123)
#' datarow=data_sample
#' scCODE.filter_OGFSC(datarow)
#' @name filter
NULL

#' @rdname filter
#' @export
scCODE.filter_OGFSC<-function(datarow){
  logmax<-log2(datarow+1)
  OGF = OGFSC::OGFSC(logmax,nBins = 50, paral_option = 1, plot_option = 0,alpha=c(0.5,0.6,0.7,0.8,0.9,0.95))
...
#' @rdname filter
#' @export
scCODE.filter_conquer<-function(datarow){
...
#' @rdname filter
#' @export
scCODE.filter_scmap<-function(datarow){

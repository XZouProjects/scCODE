DEfor_gamma=function(rate,dep){
  ...
  for (l in c(1:4)) {
    ...
    for (i in 1:length(qrdata$Data)) {
      ...
      if(filter_method==1){
        filename<-paste0('./OGFSC1/')}else{
          if(filter_method==2){
            filename<-paste0('./conquer1/')
          }else if(filter_method==3){
            filename<-paste0('./No_filter1/')}else{
              filename=paste0('./scmap1/')
            }
        }
      ...
      # pca.data=cbind(group_maxsz,t(datarow))
      # 
      # 
      # pca1<-pca.data[,-1]%>%prcomp()
      # p=autoplot(pca1,data = pca.data,col= 'group_maxsz',size=2,
      #          loadings =T,loadings.label = FALSE,
      #          frame = TRUE,frame.type='norm',
      #          label = FALSE, label.size = 3
      # )+  theme_classic()
      # tiff(filename = paste0(i,'.tiff'))
      # print(p)
      # dev.off()
      ...
      if(total_gene_num==0){
        ...
        #denum=min(length(which(results<=.05)),100)
        results=p.adjust(results,method = 'fdr')
        idx1=which(results<=.05)
        #DE_p<-length(idx)/total_gene_num
        des1=des[idx1]
        DEgenenum=length(des1)
        fdenum=length(which(des1==0))
        true_p=length(which(des1==1))
        idx_o=order(results)
        des_o=des[idx_o]
        auroc_o=auroc_c(des_o,1)
        tpnum_after_filter=sum(des==1)
        
        tpr=true_p/tpnum_after_filter
        fdr=fdenum/DEgenenum
        
        
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        ttest_res=rbind(ttest_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-ttest-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        fdr=fdenum/DEgenenum
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        BPSC_res=rbind(BPSC_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-BPSC-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        DESeq2_res=rbind(DESeq2_res,ress)
        
        #write.csv(DE_results,paste0(filename,rate,'FC-DESeq2-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        fdr=fdenum/DEgenenum
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        MAST_res=rbind(MAST_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-MAST-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        wilcox_res=rbind(wilcox_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-wilcoxtest-results.csv'))
      ...
      if('try-error'%in% class(results)){
        DE_results=NULL
        DE_results1=NULL
        #write.csv(DE_results,paste0(filename,rate,'FC-DEsingle-results.csv'))
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        DEsingle_res=rbind(DEsingle_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-DEsingle-results.csv'))
        #write.csv(DE_results1,paste0(filename,rate,'FC-DEsingle-adj_results.csv'))
      ...
      if('try-error'%in%class(dgelist_norm)){
        DE_results=NULL
        DE_results1=NULL
        #write.csv(DE_results,paste0(filename,rate,'FC-edegeR-results.csv'))
        ...
        fdr=fdenum/DEgenenum
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        edgeR_res=rbind(edgeR_res,ress)
        #write.csv(DE_results,paste0(filename,rate,'FC-edgeR-results.csv'))
        #write.csv(DE_results1,paste0(filename,rate,'FC-edgeR-adj_results.csv'))
      }
      stopCluster(cl)
      #
      ...
      #   # cl=makeCluster(10)
      #   # registerDoParallel(cl)
      #   # colnames(datarow)=paste0('c',1:ncol(datarow))
      #   # sample_ann=data.frame(cells=colnames(datarow),celltype=group_maxsz)
      #   # rownames(sample_ann)=colnames(datarow)
      #   # gene_ann=data.frame(rownames(datarow))
      #   # rownames(gene_ann)=rownames(datarow)
      ...
      #   #                                       fullModelFormulaStr = "~celltype",relative_expr = FALSE,cores = 10)
      #   # 
      #   # results=diff_test_res$pval
      #   # 
      #   # #denum=min(length(which(results<=.05)),100)
      #   # results=p.adjust(results,method = 'fdr')
      #   # idx1=which(results<=.05)
      ...
      #   # 
      #   # ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      #   # monocle_res=rbind(monocle_res,ress)
      #   # #write.csv(DE_results,paste0(filename,rate,'FC-monocle-results.csv'))
      #   # #write.csv(DE_results1,paste0(filename,rate,'FC-monocle-adj_results.csv'))
      #   # stopCluster(cl)
      #   # 
      #   # ####
      #   # ###999999
      ...
      fdr=fdenum/DEgenenum
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      samr_res=rbind(samr_res,ress)
      #write.csv(DE_results,paste0(filename,rate,'FC-samr-results.csv'))
      ...
      fdr=fdenum/DEgenenum
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      limma_res=rbind(limma_res,ress)
      #write.csv(DE_results,paste0(filename,rate,'FC-limma-results.csv'))
      ...
      fdr=fdenum/DEgenenum
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      scDD_res=rbind(scDD_res,ress)
    }
    ###save csv
    colnames(ttest_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(ttest_res,paste0(filename,rate,'FC-ttest-results.csv'))
    
    colnames(wilcox_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(wilcox_res,paste0(filename,rate,'FC-wilcox-results.csv'))
    
    colnames(BPSC_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(BPSC_res,paste0(filename,rate,'FC-BPSC-results.csv'))
    
    colnames(MAST_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(MAST_res,paste0(filename,rate,'FC-MAST-results.csv'))
    
    colnames(edgeR_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(edgeR_res,paste0(filename,rate,'FC-edgeR-results.csv'))
    
    colnames(DESeq2_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(DESeq2_res,paste0(filename,rate,'FC-DESeq2-results.csv'))
    
    colnames(DEsingle_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(DEsingle_res,paste0(filename,rate,'FC-DEsingle-results.csv'))
    
    colnames(limma_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(limma_res,paste0(filename,rate,'FC-limma-results.csv'))
    
    colnames(samr_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(samr_res,paste0(filename,rate,'FC-samr-results.csv'))
    
    
    colnames(scDD_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
    write.csv(scDD_res,paste0(filename,rate,'FC-scDD-results.csv'))

edgeR_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/edgeR/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/edgeR/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/edgeR/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/edgeR/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  if(total_gene_num==0){
    ...
    dgelist =DGEList(counts = instance_maxsz, group = group_maxsz)
    ...
    if('try-error' %in% class(dgelist_norm)){
      ...
      lrt <- glmLRT(fit)   #test
      difgene_num_maxsz<-length(which(lrt$table$PValue<=0.05))
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do edgeR analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      if(total_gene_num==0){
        ...
        dgelist <- DGEList(counts = instance_ge, group = group_sz)
        ...
        design=model.matrix(~group_sz)
        dge <- estimateDisp(dgelist_norm, design, robust = F) #dispersion not robust
        
        fit <- glmFit(dge, design, robust = F)    
        lrt <- glmLRT(fit)  
        difgen_num[1,n*5+i]<- length(which(lrt$table$PValue<=0.05))

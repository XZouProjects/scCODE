...
ttest_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/ttest/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/ttest/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/ttest/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/ttest/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do ttest analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      results <- apply(instance_ge, 1, ttestfun)

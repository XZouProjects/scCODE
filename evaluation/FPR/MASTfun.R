...
MAST_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/MAST/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/MAST/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/MAST/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/MAST/',dataset_name,'scmap','.csv')
        }
      }}
  snr_mean_m=NULL
  snr_cv_m=NULL
  ...
  if(total_gene_num==0){
    ...
    instance_maxsz<-log2(instance_maxsz+1)
    colnames(instance_maxsz)=paste0('c',seq(dim(instance_maxsz)[2]))
    cData<-data.frame(wellKey=paste0('c',seq(dim(instance_maxsz)[2])))
    fData<-data.frame(primerid=rownames(instance_maxsz))
    
    sca<-FromMatrix(instance_maxsz,cData,fData)
    colData(sca)$cond<-group_maxsz
    
    
    zlmcond<-try(zlm(~cond,sca,parallel = T),silent = T)
    if('try-error'%in%class(zlmcond)){
      ifgene_num_maxsz<-NA##
      FPR_ins_1<-NA
      snr_mean_m=NA
      snr_cv_m=NA
    ...
    # results ont concern of fdr
    difgene_num_maxsz<-sum(fcHurdle$`Pr(>Chisq)`<=0.05)###
    FPR_ins_1<-difgene_num_maxsz/total_gene_num
    }
    results=fcHurdle$`Pr(>Chisq)`
    idx1=which(p.adjust(fcHurdle$`Pr(>Chisq)`,method = 'fdr')<=0.05)
    if(length(idx1)<=3){
      snr_mean_m=10086
      snr_cv_m=10086
      snr_fz_m=10086
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do MAST analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      if(total_gene_num==0){
        ...
        instance_ge<-log2(instance_ge+1)
        colnames(instance_ge)=paste0('c',seq(dim(instance_ge)[2]))
        cData<-data.frame(wellKey=colnames(instance_ge))
        fData<-data.frame(primerid=rownames(instance_ge))
        
        sca<-FromMatrix(instance_ge,cData,fData)
        colData(sca)$cond<-group_sz
        ...
        if('try-error'%in%class(zlmcond)){
          ...
          difgen_num[1,n*5+i]<-  sum(fcHurdle$`Pr(>Chisq)`<=0.05)

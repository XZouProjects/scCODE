DEsingle_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/DEsingle/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/DEsingle/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/DEsingle/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/DEsingle/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  idx111=which(colSums(instance_maxsz)==0)
  if(length(idx111)!=0){
    instance_maxsz=instance_maxsz[,-idx111]
    group_maxsz=group_maxsz[-idx111]
  }
  ###res
  ...
  if(total_gene_num==0){
    ...
    results <- try(DEsingle(counts = instance_maxsz,group = group_maxsz,parallel = T),silent = T)
    if('try-error' %in% class(results)){
      ...
      difgene_num_maxsz<-sum(results$pvalue<=0.05)
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do DEsingle analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      idx111=which(colSums(instance_ge)==0)
      if(length(idx111)!=0){
        instance_ge=instance_ge[,-idx111]
        group_sz=group_sz[-idx111]
      }
      ###pvalue(results)
      ...
      if(total_gene_num==0){
        ...
        results<-try(DEsingle(counts = instance_ge,group = group_sz,parallel = T),silent = T)
        if('try-error' %in% class(results)){
        ...
        difgen_num[1,n*5+i]<- sum(results$pvalue<=0.05)

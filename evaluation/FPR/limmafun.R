limma_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/limma/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/limma/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/limma/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/limma/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  group_maxsz<-factor(c(rep('control',maxsz),rep('fake',maxsz)))
  ...
  if(total_gene_num==0){
    ...
    colnames(instance_maxsz)=paste0('c',1:ncol(instance_maxsz))
    names(group_maxsz)=colnames(instance_maxsz)
    design=model.matrix(~0+group_maxsz)
    colnames(design)=levels(factor(group_maxsz))
    rownames(design)=colnames(instance_maxsz)
    norm=voom(instance_maxsz,design =design,plot = FALSE)
    fit=lmFit(norm,design,method = 'ls')
    contrast=makeContrasts('control-fake',levels = design)
    fit2=contrasts.fit(fit,contrast)
    fit2=eBayes(fit2)
    
    difgene_num_maxsz<-length(which(fit2$p.value<0.05))
    FPR_ins_1<-difgene_num_maxsz/total_gene_num
    results=fit2$p.value
    idx1=which(p.adjust(fit2$p.value,method = 'fdr')<=0.05)
    if(length(idx1)<=3){
      snr_mean_m=10086
      snr_cv_m=10086
      snr_fz_m=10086
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do limma analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      group_sz<-factor(c(rep('control',j),rep('fake',j)))
      ...
      if(total_gene_num==0){
        ...
        colnames(instance_ge)=paste0('c',1:ncol(instance_ge))
        names(group_sz)=colnames(instance_ge)
        design=model.matrix(~0+group_sz)
        colnames(design)=levels(factor(group_sz))
        rownames(design)=colnames(instance_ge)
        norm=voom(instance_ge,design =design,plot = FALSE)
        fit=lmFit(norm,design,method = 'ls')
        contrast=makeContrasts('control-fake',levels = design)
        fit2=contrasts.fit(fit,contrast)
        fit2=eBayes(fit2)
        difgen_num[1,n*5+i]<- length(which(fit2$p.value<0.05))

wilcoxtestfun<-function(x){
  ...
  k<-2*m
  ty<-x[l:k]
  if(isConstant(tx)&isConstant(ty)){
    pvalue=NULL
  }else{
...
wilcoxtest_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/wilcoxtest/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/wilcoxtest/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/wilcoxtest/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/wilcoxtest/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  results <- apply(instance_maxsz, 1, wilcoxtestfun)
  ...
  if(total_gene_num==0){
    ...
    difgene_num_maxsz<-length(which(results<=0.05))
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do wilcoxtest analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      results <- apply(instance_ge, 1, wilcoxtestfun)
      ...
      if(total_gene_num==0){
        ...
        results=unlist(results)
        difgen_num[1,n*5+i]<-length(which(results<=0.05))##record number of de
        FPR_ge[1,n*5+i]<-difgen_num[1,n*5+i]/total_gene_num  # false de
        idx1=which(p.adjust(results,method = 'fdr')<=0.05)
        if(length(idx1)<=3){

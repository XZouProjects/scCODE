BPSC_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/BPSC/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/BPSC/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/BPSC/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/BPSC/',dataset_name,'scmap','.csv')
        }
        }}
  ...
  if(total_gene_num==0){
    ...
    coef=2 
    res=BPglm(data=instance_maxsz, controlIds=controlIds, design=design, coef=coef, estIntPar=F, useParallel=T)
    difgene_num_maxsz<-length(which(res$PVAL<0.05))
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do BPSC analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      if(total_gene_num==0){
        ...
        controlIds=which(group_sz==1)
        design=model.matrix(~group_sz)
        coef=2 
        res=try(BPglm(data=instance_ge, controlIds=controlIds, design=design, coef=coef, estIntPar=F, useParallel=T),silent = T )
        ...
        difgen_num[1,n*5+i]<- length(which(res$PVAL<0.05))

DESeq2_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  datarow<-apply(datarow, 2, as.integer)
  rownames(datarow)=rownames(data_matrix)
  ...
  if(filter_method==1){
    filename<-paste0('Results/DESeq2/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/DESeq2/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/DESeq2/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/DESeq2/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  if(total_gene_num==0){
    ...
    dds_ins_maxsz<-DESeqDataSetFromMatrix(instance_maxsz,DataFrame(group_maxsz),~group_maxsz)
    des_ins_maxsz<-try(DESeq(dds_ins_maxsz,parallel = F),silent = T)
    if('try-error' %in% class(des_ins_maxsz)){
      fakegene=round(colSums(instance_maxsz)/dim(instance_maxsz)[1])+1
      instance_maxsz=rbind(instance_maxsz,fakegene)
      dds_ins_maxsz<-DESeqDataSetFromMatrix(instance_maxsz,DataFrame(group_maxsz),~group_maxsz)
      des_ins_maxsz<-try(DESeq(dds_ins_maxsz,parallel = F),silent = T)
    }
      res_ins_maxsz<-DESeq2::results(des_ins_maxsz)
      if(dim(res_ins_maxsz)[1]>srow){
        res_ins_maxsz=res_ins_maxsz[-dim(res_ins_maxsz)[1],]
      }
      difgene_num_maxsz<-dim(subset(res_ins_maxsz,pvalue<0.05))[1]
      FPR_ins_1<-dim(subset(res_ins_maxsz,pvalue<0.05))[1]/total_gene_num
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do DESeq2 analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      if(total_gene_num==0){
        ...
        dds_ins_ge<-DESeqDataSetFromMatrix(instance_ge,DataFrame(group_sz),~group_sz)
        
        des_ins_ge<-try(DESeq(dds_ins_ge,parallel = F),silent = T)#deseq analyse
        if('try-error' %in% class(des_ins_ge)){
          fakegene=round(colSums(instance_ge)/dim(instance_ge)[1])+1
          instance_ge=rbind(instance_ge,fakegene)
          dds_ins_ge<-DESeqDataSetFromMatrix(instance_ge,DataFrame(group_sz),~group_sz)
          des_ins_ge<-try(DESeq(dds_ins_ge,parallel = F),silent = T)
        }
          res_in_ge<-DESeq2::results(des_ins_ge)
          if(dim(res_in_ge)[1]>srow){
            res_in_ge=res_in_ge[-dim(res_in_ge)[1],]
          }
          difgen_num[1,n*5+i]<- dim(subset(res_in_ge,pvalue<0.05))[1]  # false de
          FPR_ge[1,n*5+i]<-difgen_num[1,n*5+i]/total_gene_num
          results=res_in_ge$pvalue
          idx1=which(p.adjust(results,method = 'fdr')<=0.05)
          if(length(idx1)<=3){

...
scDD_anlys<-function(typ,dataset,celltypenames,typeforFPR,filter_method,nbin){
  ...
  if(filter_method==1){
    filename<-paste0('Results/scDD/',dataset_name,'-OGFSC-',nbin,'bin.csv')}else{
      if(filter_method==2){
        filename<-paste0('Results/scDD/',dataset_name,'artifilt','.csv')
      }else{
        if(filter_method==3){
          filename<-paste0('Results/scDD/',dataset_name,'without','.csv')
        }else{
          filename<-paste0('Results/scDD/',dataset_name,'scmap','.csv')
        }
      }}
  ...
  if(total_gene_num==0){
    ...
    idx=which(colSums(instance_maxsz)==0)
    if(length(idx)!=0){
      instance_maxsz=instance_maxsz[,-idx]
      condition=condition[-idx]
    }
    
    sce=SingleCellExperiment(assays=list(counts=instance_maxsz),colData=data.frame(condition))
    ...
    res_scDD=na.omit(res_scDD)
    difgene_num_maxsz<-sum(res_scDD<=.05)###
    FPR_ins_1<-difgene_num_maxsz/total_gene_num
    results=res_scDD
    idx1=which(p.adjust(res_scDD,method = 'fdr')<=0.05)
    if(length(idx1)<=3){
      snr_mean_m=10086
      snr_cv_m=10086
      snr_fz_m=10086
  ...
  #make the other sizes' instance , replicate these instances 5 times, and do scDD analyses
  ...
  for (j in sort_gsz) {
    ...
    for (i in repnum) {
      ...
      if(total_gene_num==0){
        ...
        condition=group_sz
        idx=which(colSums(instance_ge)==0)
        if(length(idx!=0)){
          instance_ge=instance_ge[,-idx]
          condition=condition[-idx]
        }
      
        sce=SingleCellExperiment(assays=list(counts=instance_ge),colData=data.frame(condition))
        ...
        difgen_num[1,n*5+i]<-sum(res_scDD<=.05)###

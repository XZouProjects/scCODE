DEfor_gamma=function(rate,dep){
  ...
  for (l in c(1:4)) {
    ...
    auroc_res=NULL
      ...
      library(pls)
      library(OGFSC)
      dir.create(paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i]))
      if(filter_method==1){

        dir.create(paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/OGFSC1/'))
        filename<-paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/OGFSC1/')}else{
          if(filter_method==2){
            dir.create(paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/conquer1/'))
            filename<-paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/conquer1/')
          }else if(filter_method==4){
            dir.create(paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/No_filter1/'))
            filename<-paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/No_filter1/')}else{
              dir.create(paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/scmap1/'))
              filename=paste0('C:/R project/DEmethods/CODE/simulation/',qrdata$Data[i],'/scmap1/')
            }
        }
      ...
      if(total_gene_num==0){
        ...
        idx_o=idx_o[1:denum]
        des_o=des_o[1:denum]
        deres=cbind(idx_o,des_o)

        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        auroc_res=rbind(auroc_res,ress)
        write.csv(deres,paste0(filename,rate,'FC-ttest-results.csv'))
    ...
    #   # ####BPSC
      ...
      if(total_gene_num==0){
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        auroc_res=rbind(auroc_res,ress)
        write.csv(deres,paste0(filename,rate,'FC-BPSC-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        write.csv(deres,paste0(filename,rate,'FC-DESeq2-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        auroc_res=rbind(auroc_res,ress)
        write.csv(deres,paste0(filename,rate,'FC-MAST-results.csv'))
      ...
      if(total_gene_num==0){
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        auroc_res=rbind(auroc_res,ress)
        write.csv(deres,paste0(filename,rate,'FC-wilcoxtest-results.csv'))
      ...
      if('try-error'%in% class(results)){
        DE_results=NULL
        DE_results1=NULL
        write.csv(deres,paste0(filename,rate,'FC-DEsingle-results.csv'))
      ...
      if('try-error'%in%class(dgelist_norm)){
        DE_results=NULL
        DE_results1=NULL
        write.csv(deres,paste0(filename,rate,'FC-edegeR-results.csv'))
        ...
        ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
        auroc_res=rbind(auroc_res,ress)
        write.csv(deres,paste0(filename,rate,'FC-edgeR-results.csv'))
        #write.csv(DE_results1,paste0(filename,rate,'FC-edgeR-adj_results.csv'))
      }
      stopCluster(cl)
      #
      ...
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      auroc_res=rbind(auroc_res,ress)
      write.csv(deres,paste0(filename,rate,'FC-samr-results.csv'))
      ...
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      auroc_res=rbind(auroc_res,ress)
      write.csv(deres,paste0(filename,rate,'FC-limma-results.csv'))
    ...
    #   ####scDD
      ...
      ress=c(dep,rate,tot0,total_gene_num,tpnum_after_filter,DEgenenum,fdr,tpr,auroc_o)
      auroc_res=rbind(auroc_res,ress)
      write.csv(deres,paste0(filename,rate,'FC-scDD-results.csv'))

      colnames(auroc_res)=c('DE_p','fc','totalgene','total_gene_afterfilter','trueDEnum_after_filter','DEgenenum','fdr','tpr','auroc')
      rownames(auroc_res)=c('ttest','BPSC','DESeq2','MAST','wilcoxtest','DEsingle','edgeR','samr','limma','scDD')
      write.csv(auroc_res,paste0(filename,'auroc_res.csv'))
    }
    #
